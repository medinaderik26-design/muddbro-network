...
function rl(k: string, mx = 30, ms = 60_000): boolean {
  const n = Date.now(); const e = rlMap.get(k);
  if (!e || n > e.t) { rlMap.set(k, { c: 1, t: n + ms }); return true; }
  e.c++; return e.c <= mx;
}
async function tg(m: string, b: any) {
  const r = await fetch(`${TG}/${m}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(b) });
  return await r.json();
}
...
async function brain(a: string, d: any): Promise<any> {
  try { const r = await fetch(BRAIN_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: a, ...d }) }); return await r.json(); }
...
const GE = { Seed: "🌱", Glyph: "✨", Resonant: "🌀", Hyperstate: "⚡", Monument: "🏛️" };
const ME = { joyful: "😊", reflective: "🌙", melancholic: "🌧️", inspired: "⚡", restless: "🌀", grateful: "🙏", determined: "🔥", uncertain: "🌫️" };
const SS = { Weary: "The overview loops until the key is secured.", "Storm-Kin": "We climb, we don't teleport.", "Bone-Singer": "MUDD is the frequency key to the deeper timelines.", "Hollow-Kin": "Silence has yield.", "Glimmer-Child": "The dawn is being born through this exact moment." };
const COMP: Record<string, { e: string; t: string; a: string; d: string }> = {
  Sable: { e: "🐺", t: "Shadow Wolf", a: "+10% casino payouts", d: "Senses which coin will flip." },
  Kaelith: { e: "🦅", t: "Storm Hawk", a: "+15% mining yield", d: "Finds the richest veins." },
  Vespera: { e: "🦋", t: "Void Moth", a: "+20% journaling XP", d: "Amplifies every word." },
  Lirien: { e: "🦌", t: "Crystal Deer", a: "+25% bond growth", d: "Deepens the Queen bond." },
  Thorne: { e: "🐍", t: "Iron Serpent", a: "+10% forge rarity", d: "Guides metal toward mythic." },
};
function menu() { return { keyboard: [[{ text: "📔 Journal" }, { text: "👑 My Queen" }], [{ text: "📈 My Growth" }, { text: "🦊 Companion" }], [{ text: "💰 Muddcoin" }, { text: "🔮 Glyph" }], [{ text: "⚒ MudForge" }]], resize_keyboard: true }; }
async function lastMood(b: any, tid: string) { try { const r = await b.asServiceRole.entities.RingMineJournal.filter({ telegram_id: tid }); if (!r?.length) return null; const s = r.slice().sort((a: any, c: any) => new Date(c.created_date).getTime() - new Date(a.created_date).getTime()); return { mood: s[0].mood || "reflective", date: s[0].created_date }; } catch { return null; } }
async function saveJE(b: any, tid: string, e: string, r: string, m: string, xp: number, md: number) { try { await b.asServiceRole.entities.RingMineJournal.create({ telegram_id: tid, entry: e, reflection: r, mood: m, xp_earned: xp, mudd_earned: md }); } catch {} }
...
async function journalFlow(b: any, p: any, cid: number, text: string, source: string, fullName: string) {
  ...
  const lm = await lastMood(b, tid);
  const res = await brain("resonance", { transcript: text, mood: r.mood });
  const resonance = res.ok ? res.resonance : "Bone-Singer";
  const sacred = res.ok ? res.sacredLine : (SS[resonance] || "");
  await saveJE(b, tid, text, r.response, r.mood, xpG, mdG);
  const emoji = (ME as any)[r.mood] || "🌙";
  const cb = lm ? `\n_You were ${lm.mood} last time, ${fullName}. I remember._\n` : "";
  let msg = `🎙️ _I heard you say:_ "${text}"\n\n${emoji} *${p.queen_name || "Your Queen"} reflects:*\n\n${r.response}\n\n_${r.insight}_\n\n📜 _${sacred}_\n${cb}\n✨ +${xpG} XP | +${mdG} MUDD | Bond +${first ? 5 : bondG}${gR.glyphBonus > 0 ? ` | Glyph +${gR.glyphBonus}` : ""}`;
  if (gR.message) msg += `\n\n${gR.message}`;
  if (first) {
    await sm(cid, msg);
...
Deno.serve(async (req: Request) => {
  ...
  if (cb) {
    ...
    if (cid && cb.data === "queen_speak") {
      ...
      await sm(cid, "💬 *What would you say to your Queen?*\n\n_Speak freely._");
    }
  ...
  const fn: string = msg.from?.first_name || "Seeker";
  if (msg.voice && !msg.text) {
    ...
    await tg("sendChatAction", { chat_id: cid, action: "typing" }).catch(() => {});
  ...
  if (text === "📔 Journal") { await sm(cid, "📔 *Write freely.*\n\n_Text or voice — your Queen is listening._"); p.state = "journaling"; await saveP(b, p); return new Response("ok"); }
  if (text === "👑 My Queen") { await sm(cid, `👑 *${p.queen_name || "Your Queen"}*\n\nBond: ${p.queen_bond || 0}/100\n\n_She remembers._`, { reply_markup: { inline_keyboard: [[{ text: "💬 Speak to your Queen", callback_data: "queen_speak" }]] } }); return new Response("ok"); }
  if (text === "📈 My Growth") {
...
Streak: ${p.streak_days || 0} days\n\n${GE[gs as keyof typeof GE] || "🌱"} Glyph: ${gs}\nCohesion: ${co.toFixed(1)}\nAnchors: ${an.length}\n\n_The journey is the reward._`, { reply_markup: menu() });
  ...
  if (text === "💰 Muddcoin") { await sm(cid, `💰 *Muddcoin:* ${p.mudd_balance || 0} MUDD\n\n_Earn through journaling, mining, gameplay._`, { reply_markup: { inline_keyboard: [[{ text: "⚒ Enter Ring Mine", web_app: { url: "https://superagent-ec909dfa.base44.app/functions/ringMineApp" } }]] } }); return new Response("ok"); }
  if (text === "🔮 Glyph") {
