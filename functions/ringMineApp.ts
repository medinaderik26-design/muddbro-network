...
/* QUEST */
#s-quest{background:radial-gradient(ellipse at top,#200505 0%,#060215 60%,#000 100%)}
.sw{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:12px}
.ecard{background:linear-gradient(135deg,rgba(40,5,5,0.92),rgba(12,4,22,0.92));border:1px solid rgba(160,30,30,0.4);border-radius:14px;padding:14px;text-align:center}
.eemoji{font-size:60px;line-height:1;margin-bottom:6px}
.ename{font-size:18px;font-weight:bold;color:#ff7070}
.etrad{font-size:9px;color:rgba(255,255,255,0.28);font-style:italic;margin:3px 0 6px}
.elore{font-size:11px;color:rgba(255,255,255,0.55);line-height:1.6;margin-bottom:6px;font-style:italic}
.eweak{display:inline-block;background:rgba(255,200,0,0.08);border:1px solid rgba(255,200,0,0.25);border-radius:20px;padding:3px 10px;font-size:9px;color:var(--gold);margin-bottom:10px}
.hprow{display:flex;justify-content:space-between;font-size:9px;color:rgba(255,255,255,0.28);margin-bottom:3px}
.clog{padding:8px 0;font-size:11px;color:rgba(255,255,255,0.5);font-style:italic;text-align:center;min-height:36px;line-height:1.5;white-space:pre-line}
.cbtns{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:7px}
.cb{padding:9px;border-radius:10px;border:none;font-size:11px;font-weight:bold;font-family:Georgia,serif;-webkit-appearance:none;cursor:pointer;touch-action:manipulation}
.ca{background:linear-gradient(135deg,#7a0000,#bb2020);color:#fff}
.cm{background:linear-gradient(135deg,#3a1868,#7030a0);color:#fff}
.cw{background:linear-gradient(135deg,#183a68,#3070b0);color:#fff}
.cf{background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.4);border:1px solid rgba(255,255,255,0.1)}
...
/* PEOPLE */
#s-people{background:radial-gradient(ellipse at center,#071407 0%,#040d1a 60%,#000 100%)}
.lpcard{background:linear-gradient(135deg,rgba(5,22,5,0.9),rgba(5,8,28,0.9));border:1px solid rgba(50,140,50,0.3);border-radius:14px;padding:10px;margin-bottom:8px;display:flex;align-items:center;gap:10px}
.lpem{font-size:32px;flex-shrink:0}
.lpinfo{flex:1;min-width:0}
.lpname{font-size:12px;font-weight:bold;color:#66dd66}
.lptrad{font-size:8px;color:rgba(255,255,255,0.25);text-transform:uppercase;letter-spacing:1px;margin:2px 0 3px}
.lpdesc{font-size:10px;color:rgba(255,255,255,0.5);line-height:1.4}
.lpgift{font-size:9px;color:var(--gold);margin-top:3px}
.lpbtn{padding:6px 10px;border-radius:18px;border:1px solid rgba(50,140,50,0.45);background:rgba(50,140,50,0.08);color:#66dd66;font-size:9px;font-family:Georgia,serif;flex-shrink:0;-webkit-appearance:none;cursor:pointer;touch-action:manipulation}
...
<div id="content">
  ...
  <div class="screen on" id="s-mine">
    ...
    <div class="bars">
      ...
      <div class="bar-row"><div class="bar-lbl"><span>❤ HP</span><span id="v-hp">100/100</span></div><div class="bar-track"><div class="bf b3" id="b-hp" style="width:100%"></div></div></div>
  ...
  <div class="screen" id="s-quest">
    <div class="sw">
      <div class="ecard">
        ...
        <div class="ename" id="e-name">—</div>
        <div class="etrad" id="e-trad"></div>
        <div class="elore" id="e-lore"></div>
        <div class="eweak" id="e-weak"></div>
        <div class="hprow"><span>Enemy HP</span><span id="e-hptxt">—</span></div>
        <div class="bar-track"><div class="bf b3" id="e-hpbar" style="width:100%"></div></div>
        <div class="clog" id="clog">Go to Quest tab to begin...</div>
        <div class="cbtns">
          <button class="cb ca" id="btn-atk">⚔️ Attack</button>
          <button class="cb cm" id="btn-mag">🔮 Magic</button>
          <button class="cb cw" id="btn-wit">🧠 Wit</button>
          <button class="cb cf" id="btn-run">🏃 Flee</button>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="screen" id="s-people">
    <div class="sw">
      <div class="sec">Little People — Ancient Pacts</div>
      <div id="lp-list"></div>
      <div class="sec" style="margin-top:12px">Cyber Animal Companions</div>
...
<nav class="nav">
  ...
  <button class="nb"    id="nb-quest"> <span class="ic">⚔️</span>Quest</button>
  <button class="nb"    id="nb-journal"><span class="ic">📖</span>Journal</button>
  <button class="nb"    id="nb-people"><span class="ic">👥</span>People</button>
...
var ENEMIES=[
  {name:"Wendigo",emoji:"💀",realm:1,trad:"Algonquin",hp:60,atk:18,reward:80,xp:60,lore:"Born from starvation. Once human.",weak:"Fire magic"},
  {name:"Drekavac",emoji:"🌑",realm:1,trad:"South Slavic",hp:40,atk:22,reward:50,xp:40,lore:"Its scream signals death.",weak:"Iron ore"},
  {name:"Pukwudgie",emoji:"👹",realm:1,trad:"Wampanoag",hp:35,atk:15,reward:40,xp:35,lore:"Spine-covered trickster.",weak:"Treat as equal"},
  {name:"Volkodlak",emoji:"🐺",realm:2,trad:"Slavic",hp:75,atk:25,reward:100,xp:80,lore:"A cursed human. Wolf and man war within.",weak:"Silver"},
  {name:"Koschei",emoji:"💎",realm:3,trad:"Slavic",hp:200,atk:40,reward:500,xp:300,lore:"Cannot die. His soul hidden in a needle.",weak:"Break the needle"},
  {name:"Mishipeshu",emoji:"🐆",realm:3,trad:"Ojibwe",hp:110,atk:30,reward:150,xp:120,lore:"The Great Lynx. Controls storms.",weak:"Copper"},
  {name:"Tatzelwurm",emoji:"🐉",realm:4,trad:"Alpine Germanic",hp:130,atk:35,reward:180,xp:140,lore:"Alpine forge guardian.",weak:"Rune weapons"},
  {name:"Zmey",emoji:"🐲",realm:5,trad:"Slavic",hp:250,atk:45,reward:600,xp:400,lore:"Three heads: past, present, future.",weak:"Strike all three"},
  {name:"Veles",emoji:"🌑",realm:7,trad:"Slavic God",hp:500,atk:60,reward:2000,xp:1000,lore:"God of the underworld. Tests sovereign worth.",weak:"Master all realms"}
];
var LPS=[
  {id:"c1",emoji:"🐻",name:"Bear Clan Elder",trad:"Cherokee",desc:"Guardian of mountain mines.",gift:"⛏ Mining +20%"},
  {id:"c2",emoji:"🦅",name:"Thunderbird Scout",trad:"Lakota",desc:"Reveals hidden ore veins.",gift:"🗺 Reveals next realm"},
  {id:"c3",emoji:"🌿",name:"Green Man",trad:"Germanic",desc:"Forest keeper. Heals.",gift:"❤ HP regen"},
  {id:"c4",emoji:"🐍",name:"Serpent Speaker",trad:"Slavic",desc:"Knows the undead tongue.",gift:"💀 -30% undead dmg"},
  {id:"c5",emoji:"🪶",name:"Pukwudgie Guide",trad:"Wampanoag",desc:"Once enemy, now ally.",gift:"🚪 Secret quests"},
  {id:"c6",emoji:"🌊",name:"Water Spirit",trad:"Ojibwe",desc:"Safe passage through Rusalka.",gift:"🌊 Immune to song"}
];
var COMPS=[
  {id:"vex",emoji:"🐺",name:"Vex",type:"Wolf Spirit",role:"Combat +10 vs undead"},
  {id:"aura",emoji:"🦅",name:"Aura",type:"Thunderbird",role:"Reveals hidden locations"},
  {id:"coil",emoji:"🐍",name:"Coil",type:"Serpent Elder",role:"Poison attacks"},
  {id:"kron",emoji:"🦁",name:"Kron",type:"Cyber Lion",role:"Unlocks hyperverse quests"},
  {id:"null",emoji:"🐙",name:"Null",type:"Void Crawler",role:"Data stream mining bonus"},
  {id:"pyar",emoji:"🦊",name:"Pyar",type:"Spirit Fox",role:"Doubles ore from wit wins"}
];
var MILESTONES=[
...
// ── STATE ─────────────────────────────────────────────────────
var G;
try{G=JSON.parse(localStorage.getItem("rm_v3")||"null");}catch(e){G=null;}
if(!G)G={};
G.ore=G.ore||0;G.xp=G.xp||0;G.hp=G.hp||100;G.maxhp=G.maxhp||100;
...
G.pacts=G.pacts||[];G.comps=G.comps||[];G.journals=G.journals||[];
G.last_passive=G.last_passive||Date.now();
function save(){try{localStorage.setItem("rm_v3",JSON.stringify(G));}catch(e){}}
...
function hud(){
  ...
  document.getElementById("v-realm").textContent=["","🪨","⚔️","🔮","⚙️","💻","🌌","👑"][G.realm]+" Realm "+G.realm;
  document.getElementById("v-rank").textContent=RANKS[G.realm-1];
  document.getElementById("v-xp").textContent=G.xp+" XP";
  document.getElementById("v-mp").textContent=G.power;
  document.getElementById("v-hp").textContent=G.hp+"/"+G.maxhp;
  ...
  document.getElementById("b-mine").style.width=Math.min(100,(G.power/20)*100)+"%";
  document.getElementById("b-hp").style.width=(G.hp/G.maxhp*100)+"%";
...
function doTap(cx,cy){
  var now=Date.now();
  if(now-lastTap<80)return; // debounce
  lastTap=now;

  qImg.style.transform="scale(0.87)";
  setTimeout(function(){qImg.style.transform="";},100);

  var earned=G.power+Math.floor(Math.random()*2);
...
// Use both touchstart on button AND touchstart on area as backup
tapBtn.addEventListener("touchstart",function(e){
  e.preventDefault();e.stopPropagation();
  var t=e.touches[0];doTap(t.clientX,t.clientY);
},{passive:false,capture:true});

tapBtn.addEventListener("click",function(e){
  e.preventDefault();doTap(e.clientX,e.clientY);
},{capture:true});

// Backup directly on the tap-area
...
document.getElementById("nb-quest").addEventListener("click",function(){goTo("quest");});
document.getElementById("nb-journal").addEventListener("click",function(){goTo("journal");});
document.getElementById("nb-people").addEventListener("click",function(){goTo("people");});
document.getElementById("nb-stats").addEventListener("click",function(){goTo("stats");});

// ── QUEST ─────────────────────────────────────────────────────
var CE=null,eHP=0;
function initQuest(){
  var pool=ENEMIES.filter(function(e){return e.realm<=G.realm;});
  CE=pool[Math.floor(Math.random()*pool.length)];eHP=CE.hp;
  document.getElementById("e-emoji").textContent=CE.emoji;
  document.getElementById("e-name").textContent=CE.name;
  document.getElementById("e-trad").textContent=CE.trad;
  document.getElementById("e-lore").textContent=CE.lore;
  document.getElementById("e-weak").textContent="⚡ "+CE.weak;
  document.getElementById("e-hptxt").textContent=eHP+"/"+CE.hp;
  document.getElementById("e-hpbar").style.width="100%";
  document.getElementById("clog").textContent="A "+CE.name+" blocks your path...";
}
function fight(action){
  if(!CE||eHP<=0){initQuest();return;}
  if(action==="flee"){document.getElementById("clog").textContent="You escape into the dark.";setTimeout(initQuest,1200);return;}
  var dmg=action==="attack"?8+Math.floor(Math.random()*10)+G.power:action==="magic"?12+Math.floor(Math.random()*15)+G.realm*2:5+Math.floor(Math.random()*20);
  eHP=Math.max(0,eHP-dmg);
  var icons={attack:"⚔️",magic:"🔮",wit:"🧠"};
  var log=icons[action]+" You deal "+dmg+" damage!";
  document.getElementById("e-hpbar").style.width=(eHP/CE.hp*100)+"%";
  document.getElementById("e-hptxt").textContent=eHP+"/"+CE.hp;
  if(eHP<=0){
    G.ore+=CE.reward;G.xp+=CE.xp;G.bond=Math.min(100,G.bond+3);checkMS();
    document.getElementById("clog").textContent=log+"\\n"+CE.name+" DEFEATED! +"+CE.reward+" ore";
    save();hud();setTimeout(initQuest,2000);return;
  }
  var edm=Math.floor(CE.atk*(0.5+Math.random()*0.8));
  G.hp=Math.max(0,G.hp-edm);
  if(G.hp<=0){G.hp=G.maxhp;log+="\\nYou fell — HP restored.";}
  document.getElementById("clog").textContent=log+"\\n"+CE.name+" hits you for "+edm+"!";
  save();hud();
}
document.getElementById("btn-atk").addEventListener("click",function(){fight("attack");});
document.getElementById("btn-mag").addEventListener("click",function(){fight("magic");});
document.getElementById("btn-wit").addEventListener("click",function(){fight("wit");});
document.getElementById("btn-run").addEventListener("click",function(){fight("flee");});
...
function renderPeople(){
  document.getElementById("lp-list").innerHTML=LPS.map(function(l){
    var b=G.pacts.indexOf(l.id)>-1;
    return '<div class="lpcard"><div class="lpem">'+l.emoji+'</div><div class="lpinfo"><div class="lpname">'+l.name+'</div><div class="lptrad">'+l.trad+'</div><div class="lpdesc">'+l.desc+'</div><div class="lpgift">'+l.gift+'</div></div><button class="lpbtn" data-pact="'+l.id+'">'+(b?"✅ Bonded":"Form Pact")+'</button></div>';
  }).join("");
  document.getElementById("comp-list").innerHTML=COMPS.map(function(c){
    var on=G.comps.indexOf(c.id)>-1;
    return '<div class="lpcard"><div class="lpem">'+c.emoji+'</div><div class="lpinfo"><div class="lpname">'+c.name+'</div><div class="lptrad">'+c.type+'</div><div class="lpdesc">'+c.role+'</div></div><button class="lpbtn" data-comp="'+c.id+'">'+(on?"✅ Active":"Activate")+'</button></div>';
  }).join("");
  document.querySelectorAll("[data-pact]").forEach(function(btn){
    btn.addEventListener("click",function(){
      var id=this.getAttribute("data-pact");
      if(G.pacts.indexOf(id)<0){G.pacts.push(id);G.bond=Math.min(100,G.bond+5);save();renderPeople();toast("Pact formed! +5 Bond");checkMS();}
    });
  });
  document.querySelectorAll("[data-comp]").forEach(function(btn){
    btn.addEventListener("click",function(){
      var id=this.getAttribute("data-comp"),i=G.comps.indexOf(id);
      if(i>-1)G.comps.splice(i,1);else G.comps.push(id);
      save();renderPeople();toast(i>-1?"Deactivated":"Companion activated!");
    });
  });
}
...
(function(){
  ...
  if(el>0){var g=el*(G.power+G.realm);G.ore+=g;G.last_passive=Date.now();if(g>0)setTimeout(function(){toast("Mined "+g+" ore while away!");},900);save();}
  setInterval(function(){G.ore+=(G.power+G.realm);G.last_passive=Date.now();save();hud();},60000);
})();
