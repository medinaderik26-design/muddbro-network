...
/* MUDFORGE */
#s-forge{background:radial-gradient(ellipse at top,#0a0a1a 0%,#050308 60%,#000 100%)}
.fg-scroll{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:12px}
.fg-tabs{display:flex;gap:4px;margin-bottom:10px}
.fg-tab{flex:1;padding:8px;background:rgba(255,255,255,0.04);border:1px solid var(--border);border-radius:8px;color:rgba(160,240,255,0.4);font-size:10px;text-transform:uppercase;text-align:center;font-family:'Courier New',monospace;cursor:pointer;-webkit-appearance:none}
...
.fg-card img{width:100%;border-radius:8px;margin-bottom:6px;aspect-ratio:1;object-fit:cover}
...
.fg-card .fg-rarity{position:absolute;top:6px;right:6px;font-size:7px;padding:2px 6px;border-radius:8px;text-transform:uppercase;letter-spacing:1px}
...
.fg-btn{width:100%;padding:7px;border:1px solid var(--cyan);border-radius:8px;background:transparent;color:var(--cyan);font-size:10px;font-weight:bold;font-family:'Courier New',monospace;cursor:pointer;-webkit-appearance:none;touch-action:manipulation}
...
.fg-price{font-size:12px;color:var(--gold);font-weight:bold;margin-bottom:4px}
.fg-empty{text-align:center;padding:30px 20px;color:rgba(160,240,255,0.2);font-size:11px}
...
.fg-list-form input:focus{border-color:var(--cyan)}
.fg-loading{text-align:center;padding:20px;color:rgba(160,240,255,0.3);font-size:11px}
</style>
</head>
<body>
...
<div id="content">
  ...
  <div id="s-forge" class="screen">
    <div class="fg-scroll">
      <div class="fg-tabs">
        <button class="fg-tab on" data-fgtab="shop" onclick="forgeTab('shop')">Shop</button>
        <button class="fg-tab" data-fgtab="mygear" onclick="forgeTab('mygear')">My Gear</button>
        <button class="fg-tab" data-fgtab="market" onclick="forgeTab('market')">Market</button>
      </div>
...
var FORGE_GEAR = [
  ...
  {id:"queens_mask",name:"Queen's Obsidian Mask",tribe:"Queen's Protocol",rarity:"Mythic",slot:"Head",tier:5,mining:50,companion:20,racing:10,cost:50000,
...
function renderShop() {
  ...
  FORGE_GEAR.forEach(function(g) {
    ...
    card.innerHTML = '<div class="fg-rarity ' + g.rarity + '">' + g.rarity + '</div>' +
      ...
      '<button class="fg-btn" data-buy-gear="' + g.id + '" ' + (S.ore < g.cost ? 'disabled' : '') + '>Buy & Mint NFT</button>';
    grid.appendChild(card);
  });
}

// Event delegation for shop buy buttons
document.addEventListener('click', function(e) {
  var buyBtn = e.target.closest('[data-buy-gear]');
  if (buyBtn) {
    var gearId = buyBtn.getAttribute('data-buy-gear');
    buyForgeGear(gearId);
    return;
  }
  var equipBtn = e.target.closest('[data-equip-gear]');
  if (equipBtn) {
    var gid = equipBtn.getAttribute('data-equip-gear');
    var eq = equipBtn.getAttribute('data-equipped') === 'true';
    toggleEquipGear(gid, !eq);
    return;
  }
  var sellBtn = e.target.closest('[data-sell-gear]');
  if (sellBtn) {
    var sgid = sellBtn.getAttribute('data-sell-gear');
    var sname = sellBtn.getAttribute('data-gear-name');
    openListForm(sgid, sname);
    return;
  }
  var marketBuyBtn = e.target.closest('[data-buy-market]');
  if (marketBuyBtn) {
    var lid = marketBuyBtn.getAttribute('data-buy-market');
    buyMarketItem(lid);
    return;
  }
  var cancelBtn = e.target.closest('[data-cancel-listing]');
  if (cancelBtn) {
    var clid = cancelBtn.getAttribute('data-cancel-listing');
    cancelListing(clid);
    return;
  }
  var cancelFormBtn = e.target.closest('[data-cancel-form]');
  if (cancelFormBtn) {
    var fId = cancelFormBtn.getAttribute('data-cancel-form');
    var formEl = document.getElementById('list-form-' + fId);
    if (formEl) formEl.remove();
    return;
  }
  var confirmListBtn = e.target.closest('[data-confirm-list]');
  if (confirmListBtn) {
    var lgid = confirmListBtn.getAttribute('data-confirm-list');
    var priceInput = document.getElementById('list-price-input');
    var price = parseInt(priceInput.value, 10);
    if (price && price > 0) listGearForSale(lgid, price);
    return;
  }
});
...
function buyForgeGear(gearId) {
  ...
  if(!g) return;
  if(S.ore < g.cost) return;
  S.ore -= g.cost;
  S.mudd = Math.floor(S.ore / 1000);
  // Save state (deducted ore)
  save();
  updateUI();
  // Create gear record in database
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({
      ...
      item: {
        name: g.name, image_url: g.img, tribe: g.tribe, rarity: g.rarity,
        gear_slot: g.slot, tier: g.tier,
        mining_bonus: g.mining, companion_bonus: g.companion, racing_bonus: g.racing
      }
    })
  }).then(function(r){return r.json();}).then(function(d){
    if(d.ok) {
      alert(g.name + ' acquired! NFT minted to your inventory.');
      loadMyGear();
    }
  }).catch(function(e){});
}
...
function loadMyGear() {
  ...
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'load_gear', telegram_id: telegramId})
  }).then(function(r){return r.json();}).then(function(d){
    playerGear = d.gear || [];
    renderMyGear();
  }).catch(function(e){});
...
function renderMyGear() {
  ...
  playerGear.forEach(function(g) {
    ...
    card.innerHTML = '<div class="fg-rarity ' + (g.rarity||'Common') + '">' + (g.rarity||'Common') + '</div>' +
      imgHtml +
      '<div class="fg-name">' + (g.name||'Unknown') + '</div>' +
...
function toggleEquipGear(gearId, equipped) {
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'equip_gear', telegram_id: telegramId, gear_id: gearId, equipped: equipped})
  }).then(function(r){return r.json();}).then(function(d){
    // Update local state
    playerGear.forEach(function(g){
      if(g.id === gearId) g.equipped = equipped;
    });
    renderMyGear();
  }).catch(function(e){});
}
...
function openListForm(gearId, gearName) {
  ...
  form.innerHTML = '<div style="font-size:11px;color:var(--cyan);margin-bottom:8px">List: ' + gearName + '</div>' +
    '<input type="number" id="list-price-input" placeholder="Price in MuddOre" min="1">' +
...
function listGearForSale(gearId, price) {
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'list_gear', telegram_id: telegramId, gear_id: gearId, price_mudd_ore: price, username: tgUsername})
  }).then(function(r){return r.json();}).then(function(d){
    if(d.ok) {
      alert('Listed for ' + price + ' MuddOre (' + Math.floor(price/1000) + ' MUDD)');
      loadMyGear();
    } else {
      alert('Failed to list: ' + (d.error||'unknown'));
    }
  }).catch(function(e){});
}
...
function loadMarket() {
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'load_market', telegram_id: telegramId})
  }).then(function(r){return r.json();}).then(function(d){
...
function renderMarket() {
  ...
  marketListings.forEach(function(l) {
    ...
    var buyBtn = isOwn ?
      ...
      '<button class="fg-btn" data-buy-market="' + l.id + '" ' + (S.ore < (l.price_mudd_ore||0) ? 'disabled' : '') + '>Buy (' + (l.price_mudd_ore||0) + ' Ore)</button>';
...
function buyMarketItem(listingId) {
  ...
  if(!listing) return;
  var price = listing.price_mudd_ore || 0;
  if(S.ore < price) return;
  S.ore -= price;
  S.mudd = Math.floor(S.ore / 1000);
  save();
  updateUI();
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'buy_market', telegram_id: telegramId, listing_id: listingId})
  }).then(function(r){return r.json();}).then(function(d){
    if(d.ok) {
      alert('NFT purchased! Gear transferred to your inventory.');
      loadMarket();
      loadMyGear();
    } else {
      alert('Purchase failed: ' + (d.error||'unknown'));
      // Refund
      S.ore += price;
      S.mudd = Math.floor(S.ore / 1000);
      save();
...
function cancelListing(listingId) {
  fetch(window.location.href, {
    method: 'POST',
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({action: 'cancel_listing', telegram_id: telegramId, listing_id: listingId})
  }).then(function(r){return r.json();}).then(function(d){
    if(d.ok) {
      alert('Listing cancelled. Gear returned to your inventory.');
      loadMarket();
      loadMyGear();
    }
  }).catch(function(e){});
